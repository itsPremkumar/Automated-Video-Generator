...
# 3) Run: bash broad_049_night_landscape_burned_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\broad_049_night_landscape_burned\render\_seg_broad_049_night_landscape_burned_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Broad night landscape burned","description":"Scene one from night view.

• Scene two continues.

#Shorts #AI #facts","tags":["perspnightpng","broadnightlandscape","wildperspnightpng","perspnightpngnature","perspnightpngcloseup","perspwidepng","wildperspwidepng","perspwidepngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
