...
# 3) Run: bash new_045_persp_wide_landscape_karaoke_m_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\new_045_persp_wide_landscape_karaoke_m\render\_seg_new_045_persp_wide_landscape_karaoke_m_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"New persp_wide landscape karaoke music","description":"Perspective scene one.

• Perspective scene two.

#Shorts #AI #facts","tags":["perspwidepng","perspwidelandscape","wildperspwidepng","perspwidepngnature","perspwidepngcloseup","perspanglepng","wildperspanglepng","perspanglepngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
