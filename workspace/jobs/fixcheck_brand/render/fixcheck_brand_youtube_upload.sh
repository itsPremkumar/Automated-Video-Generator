...
# 3) Run: bash fixcheck_brand_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\fixcheck_brand\render\_seg_fixcheck_brand_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Fix Check Brand Watermark","description":"Brand scene one.

• Brand scene two.

#Shorts #AI #facts","tags":["perspanglepng","checkbrandwatermark","wildperspanglepng","perspanglepngnature","perspanglepngcloseup","perspnightpng","wildperspnightpng","perspnightpngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
