...
# 3) Run: bash new_049_persp_wide_square_burned_m_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\new_049_persp_wide_square_burned_m\render\_seg_new_049_persp_wide_square_burned_m_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"New persp_wide square burned music","description":"Perspective scene one.

• Perspective scene two.

#Shorts #AI #facts","tags":["perspwidepng","perspwidesquare","wildperspwidepng","perspwidepngnature","perspwidepngcloseup","perspanglepng","wildperspanglepng","perspanglepngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
