...
# 3) Run: bash broad_013_closeup_landscape_burned_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\broad_013_closeup_landscape_burned\render\_seg_broad_013_closeup_landscape_burned_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Broad closeup landscape burned","description":"Scene one from closeup view.

• Scene two continues.

#Shorts #AI #facts","tags":["perspcloseuppng","broadcloseuplandscape","wildperspcloseuppng","perspcloseuppngnature","perspcloseuppngcloseup","perspwarmpng","wildperspwarmpng","perspwarmpngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
