...
# 3) Run: bash broad_009_aerial_square_none_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\broad_009_aerial_square_none\render\_seg_broad_009_aerial_square_none_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Broad aerial square none","description":"Scene one from aerial view.

• Scene two continues.

#Shorts #AI #facts","tags":["perspaerialpng","broadaerialsquare","wildperspaerialpng","perspaerialpngnature","perspaerialpngcloseup","perspwidepng","wildperspwidepng","perspwidepngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
