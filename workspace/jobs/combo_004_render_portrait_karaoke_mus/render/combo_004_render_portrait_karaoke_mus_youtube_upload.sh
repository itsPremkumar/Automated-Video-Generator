...
# 3) Run: bash combo_004_render_portrait_karaoke_mus_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\combo_004_render_portrait_karaoke_mus\render\_seg_combo_004_render_portrait_karaoke_mus_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Render portrait karaoke music","description":"First scene of the test.
