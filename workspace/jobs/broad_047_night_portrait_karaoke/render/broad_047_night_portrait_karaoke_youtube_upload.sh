...
# 3) Run: bash broad_047_night_portrait_karaoke_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\broad_047_night_portrait_karaoke\render\_seg_broad_047_night_portrait_karaoke_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Broad night portrait karaoke","description":"Scene one from night view.

• Scene two continues.

#Shorts #AI #facts","tags":["perspnightpng","broadnightportrait","wildperspnightpng","perspnightpngnature","perspnightpngcloseup","perspaerialpng","wildperspaerialpng","perspaerialpngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
