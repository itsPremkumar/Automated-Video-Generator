...
# 3) Run: bash broad_043_top_square_burned_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\broad_043_top_square_burned\render\_seg_broad_043_top_square_burned_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Broad top square burned","description":"Scene one from top view.

• Scene two continues.

#Shorts #AI #facts","tags":["persptoppng","broadsquareburned","wildpersptoppng","persptoppngnature","persptoppngcloseup","perspwarmpng","wildperspwarmpng","perspwarmpngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
