...
# 3) Run: bash fixcheck_portrait_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\fixcheck_portrait\render\_seg_fixcheck_portrait_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Fix Check Portrait","description":"Portrait scene one.

• Portrait scene two.

#Shorts #AI #facts","tags":["perspcloseuppng","checkportrait","wildperspcloseuppng","perspcloseuppngnature","perspcloseuppngcloseup","persptoppng","wildpersptoppng","persptoppngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
