...
# 3) Run: bash broad_001_aerial_portrait_burned_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\broad_001_aerial_portrait_burned\render\_seg_broad_001_aerial_portrait_burned_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Broad aerial portrait burned","description":"Scene one from aerial view.

• Scene two continues.

#Shorts #AI #facts","tags":["perspaerialpng","broadaerialportrait","wildperspaerialpng","perspaerialpngnature","perspaerialpngcloseup","persptoppng","wildpersptoppng","persptoppngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
