...
# 3) Run: bash fixcheck_square_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\fixcheck_square\render\_seg_fixcheck_square_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Fix Check Square","description":"Square scene one.

• Square scene two.

#Shorts #AI #facts","tags":["perspnaturepng","checksquare","wildperspnaturepng","perspnaturepngnature","perspnaturepngcloseup","perspurbanpng","wildperspurbanpng","perspurbanpngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
