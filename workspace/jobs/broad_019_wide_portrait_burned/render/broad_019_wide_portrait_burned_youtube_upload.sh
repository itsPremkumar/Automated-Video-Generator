...
# 3) Run: bash broad_019_wide_portrait_burned_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\broad_019_wide_portrait_burned\render\_seg_broad_019_wide_portrait_burned_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Broad wide portrait burned","description":"Scene one from wide view.

• Scene two continues.

#Shorts #AI #facts","tags":["perspwidepng","broadwideportrait","wildperspwidepng","perspwidepngnature","perspwidepngcloseup","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
