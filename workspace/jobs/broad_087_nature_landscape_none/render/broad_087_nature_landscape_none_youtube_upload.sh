...
# 3) Run: bash broad_087_nature_landscape_none_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\broad_087_nature_landscape_none\render\_seg_broad_087_nature_landscape_none_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Broad nature landscape none","description":"Scene one from nature view.

• Scene two continues.

#Shorts #AI #facts","tags":["perspnaturepng","broadnaturelandscape","wildperspnaturepng","perspnaturepngnature","perspnaturepngcloseup","perspaerialpng","wildperspaerialpng","perspaerialpngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
