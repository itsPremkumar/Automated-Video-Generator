...
# 3) Run: bash broad_041_top_landscape_karaoke_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\broad_041_top_landscape_karaoke\render\_seg_broad_041_top_landscape_karaoke_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Broad top landscape karaoke","description":"Scene one from top view.

• Scene two continues.

#Shorts #AI #facts","tags":["persptoppng","broadlandscapekaraoke","wildpersptoppng","persptoppngnature","persptoppngcloseup","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
