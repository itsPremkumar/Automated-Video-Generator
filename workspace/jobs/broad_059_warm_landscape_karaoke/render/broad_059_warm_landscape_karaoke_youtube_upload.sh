...
# 3) Run: bash broad_059_warm_landscape_karaoke_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\broad_059_warm_landscape_karaoke\render\_seg_broad_059_warm_landscape_karaoke_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Broad warm landscape karaoke","description":"Scene one from warm view.

• Scene two continues.

#Shorts #AI #facts","tags":["perspwarmpng","broadwarmlandscape","wildperspwarmpng","perspwarmpngnature","perspwarmpngcloseup","perspwidepng","wildperspwidepng","perspwidepngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
