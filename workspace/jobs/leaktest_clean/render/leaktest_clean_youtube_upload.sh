...
# 3) Run: bash leaktest_clean_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\leaktest_clean\render\_seg_leaktest_clean_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Leak Test Clean","description":"Clean scene one.

• Clean scene two.

#Shorts #AI #facts","tags":["perspcleanpng","leaktestclean","wildperspcleanpng","perspcleanpngnature","perspcleanpngcloseup","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
