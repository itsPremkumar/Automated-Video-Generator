...
# 3) Run: bash combo_013_render_square_burned_nomus_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\combo_013_render_square_burned_nomus\render\_seg_combo_013_render_square_burned_nomus_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Render square burned nomusic","description":"First scene of the test.
