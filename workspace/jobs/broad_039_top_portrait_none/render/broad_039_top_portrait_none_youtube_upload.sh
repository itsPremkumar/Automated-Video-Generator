...
# 3) Run: bash broad_039_top_portrait_none_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\broad_039_top_portrait_none\render\_seg_broad_039_top_portrait_none_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Broad top portrait none","description":"Scene one from top view.

• Scene two continues.

#Shorts #AI #facts","tags":["persptoppng","broadportraitnone","wildpersptoppng","persptoppngnature","persptoppngcloseup","perspwidepng","wildperspwidepng","perspwidepngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
