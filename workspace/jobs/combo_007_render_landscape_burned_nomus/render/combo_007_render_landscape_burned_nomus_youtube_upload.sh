...
# 3) Run: bash combo_007_render_landscape_burned_nomus_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\combo_007_render_landscape_burned_nomus\render\_seg_combo_007_render_landscape_burned_nomus_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Render landscape burned nomusic","description":"First scene of the test.

• Second scene continues.

#Shorts #AI #facts","tags":["githubprofilepng","renderlandscapeburned","wildgithubprofilepng","githubprofilepngnature","githubprofilepngcloseup","logoautomationpng","wildlogoautomationpng","logoautomationpngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
