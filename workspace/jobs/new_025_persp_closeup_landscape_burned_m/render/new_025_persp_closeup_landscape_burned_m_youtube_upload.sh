...
# 3) Run: bash new_025_persp_closeup_landscape_burned_m_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\new_025_persp_closeup_landscape_burned_m\render\_seg_new_025_persp_closeup_landscape_burned_m_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"New persp_closeup landscape burned music","description":"Perspective scene one.

• Perspective scene two.

#Shorts #AI #facts","tags":["perspcloseuppng","perspcloseuplandscape","wildperspcloseuppng","perspcloseuppngnature","perspcloseuppngcloseup","perspwidepng","wildperspwidepng","perspwidepngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
