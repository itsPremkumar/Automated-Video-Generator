...
# 3) Run: bash broad_stress_alltags_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\broad_stress_alltags\render\_seg_broad_stress_alltags_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Broad Stress All Tags","description":"Stress scene one.

• Stress scene two.

#Shorts #AI #facts","tags":["perspwidepng","broadstresstags","wildperspwidepng","perspwidepngnature","perspwidepngcloseup","perspnaturepng","wildperspnaturepng","perspnaturepngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
