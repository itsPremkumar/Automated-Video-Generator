...
# 3) Run: bash voice_hiin_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\voice_hiin\render\_seg_voice_hiin_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Voice hi-IN","description":"Voiceover scene one in hi-IN.

• Voiceover scene two.

#Shorts #AI #facts","tags":["perspaerialpng","voice","wildperspaerialpng","perspaerialpngnature","perspaerialpngcloseup","perspcloseuppng","wildperspcloseuppng","perspcloseuppngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
