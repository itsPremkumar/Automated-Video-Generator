...
# 3) Run: bash new_029_persp_closeup_landscape_none_m_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\new_029_persp_closeup_landscape_none_m\render\_seg_new_029_persp_closeup_landscape_none_m_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"New persp_closeup landscape none music","description":"Perspective scene one.
