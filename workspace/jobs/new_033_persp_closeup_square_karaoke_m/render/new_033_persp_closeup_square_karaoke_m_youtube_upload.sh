...
# 3) Run: bash new_033_persp_closeup_square_karaoke_m_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\new_033_persp_closeup_square_karaoke_m\render\_seg_new_033_persp_closeup_square_karaoke_m_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"New persp_closeup square karaoke music","description":"Perspective scene one.

• Perspective scene two.

#Shorts #AI #facts","tags":["perspcloseuppng","perspcloseupsquare","wildperspcloseuppng","perspcloseuppngnature","perspcloseuppngcloseup","perspwidepng","wildperspwidepng","perspwidepngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
