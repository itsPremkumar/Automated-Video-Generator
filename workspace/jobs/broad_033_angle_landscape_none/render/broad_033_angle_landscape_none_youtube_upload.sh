...
# 3) Run: bash broad_033_angle_landscape_none_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\broad_033_angle_landscape_none\render\_seg_broad_033_angle_landscape_none_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Broad angle landscape none","description":"Scene one from angle view.

• Scene two continues.

#Shorts #AI #facts","tags":["perspanglepng","broadanglelandscape","wildperspanglepng","perspanglepngnature","perspanglepngcloseup","perspwarmpng","wildperspwarmpng","perspwarmpngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
