...
# 3) Run: bash broad_075_urban_portrait_none_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\broad_075_urban_portrait_none\render\_seg_broad_075_urban_portrait_none_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Broad urban portrait none","description":"Scene one from urban view.

• Scene two continues.

#Shorts #AI #facts","tags":["perspurbanpng","broadurbanportrait","wildperspurbanpng","perspurbanpngnature","perspurbanpngcloseup","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
