...
# 3) Run: bash voice_frfr_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\voice_frfr\render\_seg_voice_frfr_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Voice fr-FR","description":"Voiceover scene one in fr-FR.

• Voiceover scene two.

#Shorts #AI #facts","tags":["persptoppng","voice","wildpersptoppng","persptoppngnature","persptoppngcloseup","perspnightpng","wildperspnightpng","perspnightpngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
