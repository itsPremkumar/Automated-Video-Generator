...
# 3) Run: bash voice_tain_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\voice_tain\render\_seg_voice_tain_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Voice ta-IN","description":"Voiceover scene one in ta-IN.

• Voiceover scene two.

#Shorts #AI #facts","tags":["perspwidepng","voice","wildperspwidepng","perspwidepngnature","perspwidepngcloseup","perspanglepng","wildperspanglepng","perspanglepngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
