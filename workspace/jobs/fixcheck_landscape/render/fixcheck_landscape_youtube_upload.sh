...
# 3) Run: bash fixcheck_landscape_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\fixcheck_landscape\render\_seg_fixcheck_landscape_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Fix Check Landscape","description":"Landscape scene one.

• Landscape scene two.

#Shorts #AI #facts","tags":["perspaerialpng","checklandscape","wildperspaerialpng","perspaerialpngnature","perspaerialpngcloseup","perspwidepng","wildperspwidepng","perspwidepngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
