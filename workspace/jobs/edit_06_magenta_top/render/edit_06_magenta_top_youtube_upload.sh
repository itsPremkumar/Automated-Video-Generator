...
# 3) Run: bash edit_06_magenta_top_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\edit_06_magenta_top\render\_seg_edit_06_magenta_top_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Edit magenta top","description":"Editing scene one.

• Editing scene two.

#Shorts #AI #facts","tags":["perspnightpng","editmagenta","wildperspnightpng","perspnightpngnature","perspnightpngcloseup","perspaerialpng","wildperspaerialpng","perspaerialpngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
