...
# 3) Run: bash broad_005_aerial_landscape_karaoke_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\broad_005_aerial_landscape_karaoke\render\_seg_broad_005_aerial_landscape_karaoke_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Broad aerial landscape karaoke","description":"Scene one from aerial view.

• Scene two continues.

#Shorts #AI #facts","tags":["perspaerialpng","broadaeriallandscape","wildperspaerialpng","perspaerialpngnature","perspaerialpngcloseup","perspurbanpng","wildperspurbanpng","perspurbanpngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
