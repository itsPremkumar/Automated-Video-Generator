...
# 3) Run: bash broad_051_night_landscape_none_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\broad_051_night_landscape_none\render\_seg_broad_051_night_landscape_none_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Broad night landscape none","description":"Scene one from night view.

• Scene two continues.

#Shorts #AI #facts","tags":["perspnightpng","broadnightlandscape","wildperspnightpng","perspnightpngnature","perspnightpngcloseup","persptoppng","wildpersptoppng","persptoppngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
