...
# 3) Run: bash broad_031_angle_landscape_burned_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\broad_031_angle_landscape_burned\render\_seg_broad_031_angle_landscape_burned_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Broad angle landscape burned","description":"Scene one from angle view.

• Scene two continues.

#Shorts #AI #facts","tags":["perspanglepng","broadanglelandscape","wildperspanglepng","perspanglepngnature","perspanglepngcloseup","persptoppng","wildpersptoppng","persptoppngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
