...
# 3) Run: bash broad_089_nature_square_karaoke_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\broad_089_nature_square_karaoke\render\_seg_broad_089_nature_square_karaoke_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Broad nature square karaoke","description":"Scene one from nature view.

• Scene two continues.

#Shorts #AI #facts","tags":["perspnaturepng","broadnaturesquare","wildperspnaturepng","perspnaturepngnature","perspnaturepngcloseup","perspwidepng","wildperspwidepng","perspwidepngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
