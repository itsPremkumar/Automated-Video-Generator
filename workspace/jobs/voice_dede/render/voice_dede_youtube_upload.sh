...
# 3) Run: bash voice_dede_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\voice_dede\render\_seg_voice_dede_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Voice de-DE","description":"Voiceover scene one in de-DE.

• Voiceover scene two.

#Shorts #AI #facts","tags":["perspwarmpng","voice","wildperspwarmpng","perspwarmpngnature","perspwarmpngcloseup","perspcoolpng","wildperspcoolpng","perspcoolpngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
