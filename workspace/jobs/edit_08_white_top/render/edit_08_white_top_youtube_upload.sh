...
# 3) Run: bash edit_08_white_top_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\edit_08_white_top\render\_seg_edit_08_white_top_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Edit white top","description":"Editing scene one.

• Editing scene two.

#Shorts #AI #facts","tags":["perspcoolpng","editwhite","wildperspcoolpng","perspcoolpngnature","perspcoolpngcloseup","perspwidepng","wildperspwidepng","perspwidepngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
