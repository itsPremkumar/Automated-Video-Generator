...
# 3) Run: bash edit_03_yellow_center_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\edit_03_yellow_center\render\_seg_edit_03_yellow_center_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Edit yellow center","description":"Editing scene one.

• Editing scene two.

#Shorts #AI #facts","tags":["perspwidepng","edityellowcenter","wildperspwidepng","perspwidepngnature","perspwidepngcloseup","perspcoolpng","wildperspcoolpng","perspcoolpngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
