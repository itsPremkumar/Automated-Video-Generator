...
# 3) Run: bash new_013_persp_aerial_square_burned_m_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\new_013_persp_aerial_square_burned_m\render\_seg_new_013_persp_aerial_square_burned_m_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"New persp_aerial square burned music","description":"Perspective scene one.

• Perspective scene two.

#Shorts #AI #facts","tags":["perspaerialpng","perspaerialsquare","wildperspaerialpng","perspaerialpngnature","perspaerialpngcloseup","perspcloseuppng","wildperspcloseuppng","perspcloseuppngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
