...
# 3) Run: bash broad_035_angle_square_karaoke_youtube_upload.sh
set -e
: "${YOUTUBE_ACCESS_TOKEN:?set YOUTUBE_ACCESS_TOKEN to your OAuth access token}"
VIDEO="C:\one\Automated-Video-Generator\workspace\jobs\broad_035_angle_square_karaoke\render\_seg_broad_035_angle_square_karaoke_0.mp4"
curl -s -X POST "https://www.googleapis.com/upload/youtube/v3/videos?part=snippet,status&uploadType=resumable" \
  ...
  -d '{"snippet":{"title":"Broad angle square karaoke","description":"Scene one from angle view.

• Scene two continues.

#Shorts #AI #facts","tags":["perspanglepng","broadanglesquare","wildperspanglepng","perspanglepngnature","perspanglepngcloseup","perspurbanpng","wildperspurbanpng","perspurbanpngnature","ai","shorts","viral"]},"status":{"privacyStatus":"private"}}' \
