...
function fontSafe(): string {
    const c = ['C:/Windows/Fonts/arial.ttf', 'C:/Windows/Fonts/seguiemj.ttf', '/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf'];
    return c.find((f) => fs.existsSync(f)) ?? 'Arial';
}
function escape(t: string): string {
    return t.replace(/:/g, '\\:').replace(/'/g, "'\\''").replace(/"/g, '\\"').replace(/,/g, '\\,').replace(/\\/g, '/');
}

export interface OverlayResult { ok: boolean; output?: string; detail: string; }

/** Burn a small corner watermark (text label). */
export async function addWatermark(file: string, label: string, out?: string, position: 'br' | 'bl' | 'tr' | 'tl' = 'br'): Promise<OverlayResult> {
    ...
    const font = fontSafe().replace(/\\/g, '/');
    const vf = `drawtext=fontfile='${font}':text='${escape(label)}':fontcolor=white@0.7:fontsize=28:x=${x}:y=${y}`;
    const { code, out: log } = await runFfmpeg(['-i', file, '-vf', vf, '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-c:a', 'copy', '-y', output]);
...
export async function addLowerThird(file: string, text: string, out?: string): Promise<OverlayResult> {
    ...
    fs.mkdirSync(path.dirname(output), { recursive: true });
    const font = fontSafe().replace(/\\/g, '/');
    const vf = `drawbox=x=0:y=h-140:w=iw:h=140:color=black@0.55:t=fill,drawtext=fontfile='${font}':text='${escape(text)}':fontcolor=white:fontsize=30:x=30:y=h-100`;
    const { code, out: log } = await runFfmpeg(['-i', file, '-vf', vf, '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-c:a', 'copy', '-y', output]);
...
export async function addProgressBar(file: string, out?: string): Promise<OverlayResult> {
    ...
    const durStr = require('child_process').execFileSync(ffprobe, ['-v', 'error', '-show_entries', 'format=duration', '-of', 'default=nw=1:nk=1', file], { encoding: 'utf-8' }) || '10';
