...
export interface MotionResult { ok: boolean; output?: string; detail: string; }
...
export async function slowMotion(file: string, factor = 2, out?: string): Promise<MotionResult> {
    ...
    const { code, out: log } = await runFfmpeg([
        '-i', file, '-vf', `setpts=${factor}*PTS`, '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-c:a', 'copy', '-y', output,
    ]);
...
export async function speedRamp(file: string, rampStart: number, rampEnd: number, slowFactor = 3, out?: string): Promise<MotionResult> {
    if (!fs.existsSync(file)) return { ok: false, detail: `input not found: ${file}` };
    ...
    const { code, out: log } = await runFfmpeg([
        ...
        '-map', '[v]', '-map', '0:a?', '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-y', output,
    ]);
