...
export interface ImgVideoResult { ok: boolean; output?: string; outputs?: string[]; detail: string; }
...
export async function imagesToVideo(
    ...
    if (images.length === 1) {
        ...
        const { code, out: log } = await runFfmpeg(['-loop', '1', '-i', images[0], '-vf', vf, '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-t', String(dur), '-y', out]);
    ...
    const { code, out: log } = await runFfmpeg([
        '-f', 'concat', '-safe', '0', '-i', list, '-vf', `fps=25,${vf}`,
...
export async function videoToImages(
    file: string,
    outDir?: string,
    everyNthFrame = 30,
): Promise<ImgVideoResult> {
    ...
    const { code, out: log } = await runFfmpeg([
        '-i', file, '-vf', `select='not(mod(n\\,${everyNthFrame}))',scale=480:-1`,
        '-vsync', 'vfr', '-frames:v', '10000', path.join(dir, 'frame_%05d.png'), '-y',
    ]);
    if (code !== 0) return { ok: false, detail: `video->images failed:\n${log.slice(-600)}` };
    const outputs = fs.readdirSync(dir).filter((f) => f.endsWith('.png')).map((f) => path.join(dir, f));
