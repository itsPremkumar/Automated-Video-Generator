...
export interface DemuxResult { ok: boolean; output?: string; detail: string; }

export async function extractAudioStream(file: string, out?: string, fmt: 'mp3' | 'wav' | 'm4a' = 'mp3'): Promise<DemuxResult> {
...
export async function muteVideo(file: string, out?: string): Promise<DemuxResult> {
    ...
    fs.mkdirSync(path.dirname(output), { recursive: true });
    const { code, out: log } = await runFfmpeg(['-i', file, '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-an', '-y', output]);
