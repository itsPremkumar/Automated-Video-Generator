...
export const dynamicCaptionsPlugin: AgenticPlugin = {
    metadata: { name: 'dynamic-captions', version: '1.0.0', description: 'Animated caption styles (bounce, pop, fade)', tags: ['captions', 'text'] },
    ...
    hooks: {
        onRenderFilter: async (scene, ctx) => {
            ...
            return { ...scene, filterChain: (scene.filterChain ?? '') + `,drawtext=fontsize=${cfg.fontSize ?? 42}:fontcolor=white:box=1:boxcolor=black@0.5:x=(w-text_w)/2:y=h-text_h-120` };
        },
    },
};
export function registerDynamicCaptions(r: any, c?: any, e = true) { r.register(dynamicCaptionsPlugin, c, e); }
