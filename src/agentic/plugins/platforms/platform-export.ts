...
interface PlatformConfig {
    /** Target platform */
    platform?: 'tiktok' | 'reels' | 'shorts' | 'youtube' | 'instagram' | 'linkedin' | 'twitter' | 'facebook' | 'vimeo' | 'custom';
...
const PLATFORM_SPECS: Record<string, PlatformSpec> = {
    tiktok: {
        ...
        safeZones: [
            { name: 'caption', type: 'caption', position: { x: '5%', y: '70%', w: '90%', h: '20%' }, description: 'Bottom caption area' },
            { name: 'handle', type: 'handle', position: { x: '5%', y: '5%', w: '30%', h: '8%' }, description: 'Username/profile' },
            { name: 'cta', type: 'cta', position: { x: '5%', y: '85%', w: '90%', h: '10%' }, description: 'Follow/like buttons' },
            { name: 'progress', type: 'progress', position: { x: '0%', y: '96%', w: '100%', h: '4%' }, description: 'Progress bar' },
        ],
    ...
    reels: {
        ...
        safeZones: [
            { name: 'caption', type: 'caption', position: { x: '5%', y: '72%', w: '90%', h: '18%' }, description: 'Caption overlay' },
            { name: 'handle', type: 'handle', position: { x: '5%', y: '5%', w: '25%', h: '6%' }, description: 'Profile info' },
            { name: 'music', type: 'cta', position: { x: '5%', y: '88%', w: '90%', h: '8%' }, description: 'Music attribution' },
            { name: 'ui', type: 'ui', position: { x: '85%', y: '20%', w: '12%', h: '60%' }, description: 'Right side UI (like/comment/share)' },
        ],
        thumbnail: { width: 1080, height: 1920, format: 'jpg' },
    ...
    shorts: {
        ...
        safeZones: [
            { name: 'caption', type: 'caption', position: { x: '5%', y: '75%', w: '90%', h: '15%' }, description: 'Caption area' },
            { name: 'subscribe', type: 'cta', position: { x: '5%', y: '88%', w: '90%', h: '8%' }, description: 'Subscribe button' },
            { name: 'progress', type: 'progress', position: { x: '0%', y: '95%', w: '100%', h: '5%' }, description: 'Progress indicator' },
        ],
    ...
    youtube: {
        ...
        safeZones: [
            { name: 'title', type: 'title', position: { x: '5%', y: '5%', w: '90%', h: '10%' }, description: 'Video title overlay' },
            { name: 'end-screen', type: 'cta', position: { x: '10%', y: '70%', w: '80%', h: '25%' }, description: 'End screen elements' },
        ],
        thumbnail: { width: 1280, height: 720, format: 'jpg' },
    ...
    linkedin: {
        ...
        bitrate: { video: '10M', audio: '128k' },
        safeZones: [
            { name: 'caption', type: 'caption', position: { x: '5%', y: '80%', w: '90%', h: '15%' }, description: 'Post text overlay' },
        ],
        thumbnail: { width: 1280, height: 720, format: 'jpg' },
    ...
    twitter: {
        ...
        bitrate: { video: '8M', audio: '128k' },
        safeZones: [
            { name: 'tweet', type: 'caption', position: { x: '5%', y: '75%', w: '90%', h: '15%' }, description: 'Tweet text overlay' },
        ],
        thumbnail: { width: 1280, height: 720, format: 'jpg' },
        ...
        onPostRender: async (outputPath, ctx) => {
            ...
            if (cfg.thumbnails) {
                for (const spec of specs) {
                    const thumbPath = path.join(outDir, `${baseName}_${spec.name.toLowerCase()}_thumb.${spec.thumbnail.format}`);
...
async function generateThumbnail(ffmpeg: string, input: string, output: string, spec: PlatformSpec): Promise<void> {
    ...
    await execAsync(ffmpeg, [
        ...
        '-ss', '00:00:01',
        '-vframes', '1',
        '-vf', `scale=${spec.thumbnail.width}:${spec.thumbnail.height}:force_original_aspect_ratio=decrease,pad=${spec.thumbnail.width}:${spec.thumbnail.height}:(ow-iw)/2:(oh-ih)/2`,
        '-y', output,
    ]);
}
...
async function transcodeForPlatform(
    ...
    await execAsync(ffmpegPath, [
        ...
        '-b:v', vBitrate,
        '-maxrate', vBitrate,
        '-bufsize', `${parseInt(vBitrate) * 2}k`,
        '-c:a', 'aac',
        '-b:a', aBitrate,
        '-r', String(spec.frameRate),
        '-vf', `scale='if(gt(iw,ih),${spec.aspect === '16:9' ? 1920 : 1080},-2)':'if(gt(iw,ih),-2,${spec.aspect === '16:9' ? 1080 : 1920})':force_original_aspect_ratio=decrease,pad=${spec.aspect === '16:9' ? 1920 : 1080}:${spec.aspect === '16:9' ? 1080 : 1920}:(ow-iw)/2:(oh-ih)/2`,
        '-movflags', '+faststart',
        '-y', output,
    ]);
}
