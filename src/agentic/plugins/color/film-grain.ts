...
export const filmGrainPlugin: AgenticPlugin = {
    metadata: { name: 'film-grain', version: '1.0.0', description: 'Film grain texture overlay', tags: ['color', 'grain'] },
    ...
    hooks: {
        onRenderFilter: async (scene, ctx) => {
            ...
            return { ...scene, filterChain: (scene.filterChain ?? '') + `,noise=alls=${Math.round(c.strength * 100)}:allf=t+u` };
        },
    },
};
export function registerFilmGrain(r: any, c?: any, e = true) { r.register(filmGrainPlugin, c, e); }
