...
test('A10: missing image asset -> branded placeholder PNG is created and kept', async () => {
    ...
    try {
        ...
        const out = await import('./orchestrate.js').then((m) =>
            m.prepareRemotionAssets(res, { brand: { accentColor: '#FF6B35' } }, jobDir, runFfmpeg as any),
        );
...
test('A10: present image asset -> copied as-is (no placeholder)', async () => {
    ...
    try {
        ...
        const out = await import('./orchestrate.js').then((m) =>
            m.prepareRemotionAssets(res, {}, jobDir, runFfmpeg as any),
        );
