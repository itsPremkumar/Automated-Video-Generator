...
test('searchFreeImages maps ImageResult -> MediaAsset and is network-safe', async () => {
  ...
  adapter.searchAll = async () => ([
    { source: 'wikimedia', results: [{ downloadUrl: 'http://x/a.jpg', thumbnailUrl: 'http://x/a_t.jpg', width: 1920, height: 1080, creator: 'Alice', provider: 'wikimedia' }] },
    { source: 'nasa', results: [{ downloadUrl: 'http://x/b.jpg', width: null, height: null, creator: 'NASA', provider: 'nasa' }] },
  ]);
  try {
...
test('searchFreeImages returns [] on adapter failure (safe, no throw)', async () => {
  const adapter = (proxy as any).freeImageAdapter;
  const orig = adapter.searchAll.bind(adapter);
  adapter.searchAll = async () => { throw new Error('net down'); };
  try {
