...
const SFX_SPECS: Record<SfxKind, { description: string; args: string[]; durationMs: number }> = {
    whoosh: {
        ...
        durationMs: 300,
        args: [
            '-f', 'lavfi', '-i', 'sine=frequency=200:duration=0.3,volume=0.3',
            '-af', 'vibrato=f=0.1:d=0.3,volume=0.25',
            '-c:a', 'pcm_s16le', '-y',
        ],
    },
    ding: {
        ...
        args: [
            ...
            '-af', 'afade=t=out:st=0.05:d=0.35,volume=0.3',
            '-c:a', 'pcm_s16le', '-y',
        ],
    },
    impact: {
        ...
        durationMs: 150,
        args: [
            '-f', 'lavfi', '-i', 'anoisesrc=d=0.15:c=pink:a=0.5',
            '-af', 'afade=t=in:d=0.01,afade=t=out:st=0.1:d=0.05,volume=0.35',
            '-c:a', 'pcm_s16le', '-y',
        ],
    },
    notification: {
    ...
    click: {
        ...
        args: [
            ...
            '-af', 'afade=t=in:d=0.001,afade=t=out:st=0.04:d=0.01,volume=0.2',
            '-c:a', 'pcm_s16le', '-y',
        ],
    },
    pop: {
        ...
        durationMs: 80,
        args: [
            '-f', 'lavfi', '-i', 'anoisesrc=d=0.08:c=pink:a=0.6',
            '-af', 'afade=t=in:d=0.005,afade=t=out:st=0.05:d=0.03,volume=0.25',
            '-c:a', 'pcm_s16le', '-y',
        ],
    },
    transition: {
        ...
        args: [
            ...
            '-af', 'afade=t=in:d=0.05,afade=t=out:st=0.3:d=0.1',
            '-c:a', 'pcm_s16le', '-y',
        ],
    },
    swish: {
        ...
        args: [
            ...
            '-af', 'vibrato=f=0.2:d=0.15,volume=0.15',
            '-c:a', 'pcm_s16le', '-y',
        ],
    },
    bounce: {
        ...
        args: [
            ...
            '-af', 'vibrato=f=4:d=0.35,afade=t=in:d=0.01,afade=t=out:st=0.25:d=0.1,volume=0.25',
            '-c:a', 'pcm_s16le', '-y',
        ],
    },
};
...
export class FfmpegSfxGenerator {
    ...
    async getSfx(kind: SfxKind): Promise<SfxClip | null> {
        ...
        if (cached && fs.existsSync(cached)) {
            return { kind, localPath: cached, durationMs: SFX_SPECS[kind].durationMs, description: SFX_SPECS[kind].description };
        }

        const outPath = path.join(SFX_CACHE_DIR, `${cacheKey}.wav`);
        if (fs.existsSync(outPath)) {
            this.cache.set(cacheKey, outPath);
            return { kind, localPath: outPath, durationMs: SFX_SPECS[kind].durationMs, description: SFX_SPECS[kind].description };
        }
