...
export function homePage(req: Request, videos: VideoRecord[], setup: SetupStatus, musicFiles: string[]): string {
    ...
    const body = `<section class="hero-surface"><div class="hero-grid"><div class="stack"><span class="eyebrow">Local AI Video Studio</span><div><h1>Create videos from a script, not from folders</h1><p class="lead">Paste your idea, shape the voice and layout, then let the portal handle stock visuals, narration, subtitles, rendering, and delivery in one place.</p><p class="muted">This screen is designed for normal users. No need to manually edit the input or output folders during everyday use.</p></div><div class="toolbar"><a class="button" href="#workspace">Open the workspace</a><a class="button secondary" href="${PROJECT_REPOSITORY_URL}" target="_blank" rel="noreferrer">View on GitHub</a><a class="button ghost" href="/llms.txt">Read AI summary</a></div><div class="metric-grid"><div class="metric-card"><strong>${videos.length}</strong><span class="muted">videos created in this portal</span></div><div class="metric-card"><strong>${totalVoicePresets}+</strong><span class="muted">voice presets available before dynamic loading</span></div><div class="metric-card"><strong>3 steps</strong><span class="muted">setup, create, watch or download</span></div></div></div><div class="highlight-box stack"><span class="eyebrow">Simple Flow</span><h2>What users do here</h2><div class="row">${setupSummary}</div><ol class="checklist"><li>Save the API keys once for this computer.</li><li>Paste or edit the script in the workspace below.</li><li>choose voice, layout, music, and subtitle options.</li><li>Start the render and wait on the live status page.</li><li>Watch or download the MP4 from the final delivery page.</li></ol></div></div></section><section class="layout-split"><div class="panel tint stack"><div><span class="eyebrow">One-Time Setup</span><h2>Prepare this device once</h2><p class="muted">Most users only need a Pexels API key. Save it here and the browser portal becomes the main way to use the project.</p></div><div class="row">${setupSummary}</div><div id="setup-readiness" class="status-board"></div></div><div class="panel"><form id="setup-form" class="form"><div class="field-grid two-up"><div class="field"><div class="row" style="justify-content:space-between"><label for="setup-pexels">Pexels API key</label><div class="row"><span id="setup-pexels-status" class="status-chip warn">Checking...</span><button type="button" id="setup-pexels-toggle" class="secondary" style="padding:4px 10px;font-size:12px;display:none" onclick="toggleFieldUpdate('pexels')">Change</button></div></div><input id="setup-pexels" type="password" placeholder="Recommended for stock video search"><p class="field-help">Best source for usable portrait and landscape stock footage.</p></div><div class="field"><div class="row" style="justify-content:space-between"><label for="setup-pixabay">Pixabay API key</label><div class="row"><span id="setup-pixabay-status" class="status-chip warn">Checking...</span><button type="button" id="setup-pixabay-toggle" class="secondary" style="padding:4px 10px;font-size:12px;display:none" onclick="toggleFieldUpdate('pixabay')">Change</button></div></div><input id="setup-pixabay" type="password" placeholder="Optional backup provider"><p class="field-help">Optional secondary image and video source.</p></div><div class="field"><div class="row" style="justify-content:space-between"><label for="setup-gemini">Gemini API key</label><div class="row"><span id="setup-gemini-status" class="status-chip warn">Checking...</span><button type="button" id="setup-gemini-toggle" class="secondary" style="padding:4px 10px;font-size:12px;display:none" onclick="toggleFieldUpdate('gemini')">Change</button></div></div><input id="setup-gemini" type="password" placeholder="Optional AI helper"><p class="field-help">Only needed if your workflows use Gemini-powered helpers.</p></div></div><div class="toolbar"><button type="submit">Save Setup</button><span class="muted">Launcher users can open this page from <strong>Start-Automated-Video-Generator.bat</strong>.</span></div></form><div id="setup-feedback" class="status" hidden></div></div></section><section id="workspace" class="stack" style="margin-top:20px"><div class="panel-head"><div><span class="eyebrow">Studio</span><h2>Unified Video Studio</h2><p class="muted">All your script and design controls in one place. Optimized for pro creation flow.</p></div></div><form id="generate-form" class="studio-grid"><div id="form-status" class="status" style="grid-column:1 / -1; margin-bottom:10px" hidden></div><div class="panel form-panel"><div class="panel-head"><div><span class="eyebrow">Step 1</span><h2>Script & Story</h2><p class="muted small">Write the visual instructions using <strong>[Visual: ...]</strong> cues.</p></div></div><div class="field"><label for="title">Video title</label><input id="title" value="${escapeHtml(defaultTitle)}" placeholder="How AI Is Changing Everyday Life" maxlength="${MAX_TITLE_LENGTH}" required><p class="field-help">Used for output page and filename.</p></div><div class="field"><label for="script">Input script</label><div class="script-shell"><div class="script-toolbar"><span class="muted small">Editable input area</span><div id="script-metrics" class="row"><span class="helper-badge">0 words</span><span class="helper-badge">0 sec est.</span></div></div><textarea id="script" placeholder="[Visual: futuristic robotics lab] AI is changing how people and robots work together.&#10;&#10;[Visual: doctor reviewing an AI dashboard] In healthcare, it helps spot patterns faster and supports earlier decisions." required>${escapeHtml(defaultScript)}</textarea></div></div><div class="row" style="margin-top:10px"><button type="button" id="fill-sample" class="secondary" style="font-size:13px;padding:8px 14px">Use Sample</button><button type="button" id="fill-hello" class="secondary" style="font-size:13px;padding:8px 14px">Hello World</button></div></div><div class="panel form-panel"><div><span class="eyebrow">Step 2</span><h2>Voice & Layout</h2><p class="muted small">Detect language or lock the voice yourself.</p></div><div class="stack" style="gap:12px"><div class="field"><label for="orientation">Orientation</label><select id="orientation"><option value="portrait">Portrait (9:16)</option><option value="landscape">Landscape (16:9)</option></select></div><div class="field"><label for="language">Language</label><select id="language"><option value="">Detect automatically</option>${languageOptions}</select></div><div class="field"><label for="voice-search">Search voice</label><input type="text" id="voice-search" class="voice-search" placeholder="Search voices..."></div><div class="field"><label for="voice">Voice override</label><select id="voice"><option value="">Optional Override</option>${voiceOptions}</select><p id="voice-hint" class="field-help" style="font-size:11px"></p></div></div></div><div class="panel form-panel"><div><span class="eyebrow">Step 3</span><h2>Style & Output</h2><p class="muted small">Final touches before rendering the MP4.</p></div><div class="stack" style="gap:12px"><div class="field"><label for="backgroundMusic">Background music</label><div class="row" style="flex-wrap:nowrap"><select id="backgroundMusic" style="font-size:13px"><option value="">No music</option>${musicOptions}</select><button type="button" class="secondary" onclick="openSystemBrowser('music')" style="padding:8px 12px;font-size:13px">Browse</button></div></div><div class="field"><label for="defaultVideo">Fallback asset</label><input id="defaultVideo" value="${escapeHtml(DEFAULT_FALLBACK_VIDEO)}" placeholder="Fallback asset"></div><label class="toggle-row" for="showText" style="padding:10px 12px"><input id="showText" type="checkbox" checked> <div><strong>Show subtitles</strong></div></label><div class="stack" style="margin-top:4px"><div class="row" style="justify-content:space-between;align-items:center"><strong style="font-size:14px">Local Media</strong><button type="button" class="secondary" onclick="openSystemBrowser('media')" style="padding:6px 12px;font-size:12px">Add Library</button></div><div id="asset-gallery" class="asset-gallery"></div></div></div><div class="toolbar" style="grid-column: 1 / -1; margin-top:20px; justify-content:center"><button type="submit" style="min-width:300px; padding:16px 32px">Start Rendering Video Studio</button></div></form></section><section class="layout-split"><div class="panel soft"><span class="eyebrow">Editing Tips</span><h2>Creation checklist</h2><ul class="compact-list"><li>Use one clear idea per sentence so voiceover stay readable.</li><li>Add scene hints like <strong>[Visual: modern city]</strong> for better search results.</li><li>Choose portrait for Shorts and landscape for YouTube.</li><li>Use Fallback video to prevent render failure if stock fails.</li></ul></div><div class="panel"><span class="eyebrow">Latest Outputs</span><h2>Recent finished uploads</h2><p class="muted">Quickly access the watch pages for your latest exports.</p><div class="recent-grid">${recentCards}</div></div></section><section id="recent-videos" class="panel"><div class="panel-head"><div><span class="eyebrow">Library</span><h2>Completed archives</h2><p class="muted">All your history is saved here automatically.</p></div></div><div class="cards">${cards}</div></section>
